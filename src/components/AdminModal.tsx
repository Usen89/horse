...
import { Settings, CheckCircle, AlertCircle, X, Database, Download, Upload, Cloud as CloudIcon, UploadCloud, DownloadCloud } from 'lucide-react';
 ...
 * данные аккаунта (для чтения), резервное копирование и облачная база.
 */
export default function AdminModal({ isOpen, onClose, currentAdmin }: AdminModalProps) {
  ...
  const handleCloudPush = async () => {
    ...
    setCloudBusy(false);
    setCloudMsg({ ok: res.success, text: res.success && res.details ? `${res.message} (${res.details.join(', ')})` : res.message });
  };
  ...
  const handleCloudPull = async () => {
    ...
    setCloudBusy(false);
    setCloudMsg({ ok: res.success, text: res.success && res.details ? `${res.message} (${res.details.join(', ')})` : res.message });
    if (res.success) setTimeout(() => window.location.reload(), 1400);
  };
    ...
    >
      ...
      <div className="bg-slate-900 px-6 py-5 text-white flex justify-between items-center shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium">Резервные копии и облачная база</p>
          </div>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="pt-5 border-t border-slate-100 space-y-3">
          ...
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Синхронизация между устройствами. Подключение уже настроено; при необходимости
            можно указать другой проект (Project URL и anon key из Settings → API).
          </p>
          ...
          {cloudMsg && (
            <div className={`text-xs p-3 rounded-xl flex items-start gap-2 border ${cloudMsg.ok ? 'bg-emerald-50 border-emerald-100 text-emerald-800' : 'bg-rose-50 border-rose-100 text-rose-800'}`}>
              {cloudMsg.ok ? <CheckCircle className="w-4 h-4 text-emerald-500 mt-0.5 shrink-0" /> : <AlertCircle className="w-4 h-4 text-rose-500 mt-0.5 shrink-0" />}
